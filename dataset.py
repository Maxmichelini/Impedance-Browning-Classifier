...
class AppleBrowningDataset(Dataset):
    def __init__(self, excel_file:str, sheet_name:str, transform=None):
        ...
        # METODO 1) Seleziona tutte le colonne di magnitude e phase
        ...
        #METODO 2) Seleziona le colonne di magnitude e phase tramite un ciclo
        magnitude_columns = []
        start_addingM = False
...
if __name__ == '__main__':  # Assicura che il codice al suo interno venga eseguito solo se il file viene eseguito direttamente, non se viene importato come modulo in un altro file
    excel_file = "Full Data Analysis- file, new revised_Sundus 1.xlsx"  # Scelta del file Excel
    sheet_name = "Exp. 2"  # Scelta del foglio di lavoro
    transform = None  # Non viene applicata alcuna trasformazione

    # try-except per catturare errori e fare il debug
    try:
